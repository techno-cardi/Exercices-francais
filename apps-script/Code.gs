...
function verifyTeacherPassword_(request) {
  ...
  let saved=String(user.motDePasseHash||'');
  if(saved){
...
function grade_(request, session) {
  ...
  appendObjects_(TABS.responses,details.map(detail=>({ horodatage:now, remiseId, courriel:session.email, affectationId:assignment.id, exerciceId:exerciseId, elementId:detail.id, reponse:seal_(JSON.stringify(submitted[detail.id] ?? null)), reussi:detail.correct })));
  ensureLiveSheet_();const liveKey=[session.email,assignment.id,exerciseId].join('|');withScriptLock_(()=>upsert_(TABS.live,'cle',liveKey,{cle:liveKey,horodatage:now,courriel:session.email,nom:session.name,groupes:session.groups.join(','),affectationId:assignment.id,titre:assignment.titre,matriceId:String(request.matrixId),exerciceId:exerciseId,exercice:String(request.exerciseLabel||('Exercice '+exerciseId)),reponsesJson:seal_(JSON.stringify(submitted)),reponsesDonnees:Object.keys(submitted).filter(key=>hasAnswer_(submitted[key])).length,reponsesTotal:total,etat:'remis'}));
...
function seal_(text){const secret=PropertiesService.getScriptProperties().getProperty('APP_SECRET');if(!secret)throw appError_('L’application doit être configurée.','SETUP_REQUIRED');const iv=Utilities.getUuid(),plain=Utilities.newBlob(String(text),'text/plain').getBytes(),out=[];for(let offset=0;offset<plain.length;offset+=32){const block=Utilities.computeHmacSha256Signature(iv+':'+Math.floor(offset/32),secret);for(let i=0;i<Math.min(32,plain.length-offset);i++)out.push((plain[offset+i]^block[i])&255);}const data=Utilities.base64EncodeWebSafe(out).replace(/=+$/,''),tag=Utilities.base64EncodeWebSafe(Utilities.computeHmacSha256Signature(iv+'|'+data,secret)).replace(/=+$/,'');return 'v1.'+iv+'.'+data+'.'+tag;}
function open_(sealed){const value=String(sealed||'');if(!value.startsWith('v1.'))return value;const parts=value.split('.');if(parts.length!==4)throw new Error('Donnée protégée invalide.');const secret=PropertiesService.getScriptProperties().getProperty('APP_SECRET'),expected=Utilities.base64EncodeWebSafe(Utilities.computeHmacSha256Signature(parts[1]+'|'+parts[2],secret)).replace(/=+$/,'');if(expected!==parts[3])throw new Error('Donnée protégée invalide.');const bytes=Utilities.base64DecodeWebSafe(parts[2]),out=[];for(let offset=0;offset<bytes.length;offset+=32){const block=Utilities.computeHmacSha256Signature(parts[1]+':'+Math.floor(offset/32),secret);for(let i=0;i<Math.min(32,bytes.length-offset);i++)out.push((bytes[offset+i]^block[i])&255);}return Utilities.newBlob(out).getDataAsString('UTF-8');}
function requireSession_(token){if(!token)throw appError_('Reconnecte-toi pour continuer.','AUTH_REQUIRED');const parts=String(token).split('.');if(parts.length!==2||signature_(parts[0])!==parts[1])throw appError_('Ta session est invalide.','AUTH_REQUIRED');const data=JSON.parse(Utilities.newBlob(Utilities.base64DecodeWebSafe(parts[0])).getDataAsString('UTF-8'));if(Number(data.exp)<Date.now())throw appError_('Ta session est expirée.','AUTH_REQUIRED');return data;}
function signToken_(data){const bytes=Utilities.newBlob(JSON.stringify(data),'application/json').getBytes(),payload=Utilities.base64EncodeWebSafe(bytes).replace(/=+$/,'');return payload+'.'+signature_(payload);}
...
function appendObjects_(name,objects){if(!objects.length)return;const sheet=spreadsheet_().getSheetByName(name),headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0],rows=objects.map(obj=>headers.map(header=>obj[header] ?? ''));sheet.getRange(sheet.getLastRow()+1,1,rows.length,headers.length).setValues(rows);}
