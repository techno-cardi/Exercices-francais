const TABS = { users:'Utilisateurs', groups:'Groupes', assignments:'Affectations', results:'Résultats', responses:'Réponses', answers:'Corrigés' };
...
function doPost(event) {
  try {
    ...
    const privateActions = { bootstrap: bootstrap_, dashboard: dashboard_, studentCopy: studentCopy_, saveGroup: saveGroup_, importUsers: importUsers_, saveAssignment: saveAssignment_, grade: grade_ };
    let result;
...
function bootstrap_(request, session) {
  ...
  if (session.role === 'enseignant') {
    const groups = readObjects_(TABS.groups).filter(item => isTrue_(item.actif)).map(group => ({ code:String(group.code), name:group.nom, students:countStudents_(group.code) }));
...
function studentCopy_(request, session) {
  ...
  const responses=readObjects_(TABS.responses); const remises=Object.values(latest).sort((a,b)=>(new Date(a.horodatage).getTime()||0)-(new Date(b.horodatage).getTime()||0)).map(row=>{const answers={},details={};responses.filter(item=>String(item.remiseId)===String(row.remiseId)).forEach(item=>{try{answers[item.elementId]=JSON.parse(item.reponse);}catch{answers[item.elementId]=item.reponse;}details[item.elementId]=isTrue_(item.reussi);});return {remiseId:row.remiseId,timestamp:row.horodatage,matrixId:String(row.matriceId),exerciseId:String(row.exerciceId),exerciseLabel:row.exercice,score:Number(row.score)||0,total:Number(row.total)||0,percentage:Number(row.pourcentage)||0,attempt:Number(row.tentative)||1,answers,details};});
...
function dashboardData_() {
  ...
  const progress=Object.values(latest).sort((a,b)=>(new Date(b.horodatage).getTime()||0)-(new Date(a.horodatage).getTime()||0)).map(row=>({ timestamp:row.horodatage,email:row.courriel,name:row.nom||row.courriel,groups:splitList_(row.groupes),assignmentId:String(row.affectationId),assignmentTitle:row.titre,exerciseId:String(row.exerciceId),exerciseLabel:row.exercice||('Exercice '+row.exerciceId),score:Number(row.score)||0,total:Number(row.total)||0,percentage:Number(row.pourcentage)||0,attempt:Number(row.tentative)||1 }));
  const scores=progress.map(row=>row.percentage).filter(Number.isFinite);
  return { results:{ remises:rows.length, average:scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):0, activeStudents:new Set(rows.map(row=>row.courriel)).size }, progress, updatedAt:new Date().toISOString() };
}
...
function importUsers_(request, session) {
  ...
  emails.forEach(email => { const current=findUser_(email); const groups=new Set(splitList_(current && current.groupes)); if(groupCode)groups.add(groupCode); upsert_(TABS.users,'courriel',email,{ courriel:email, nom:(current&&current.nom)||'', role, groupes:[...groups].join(','), actif:true, creeLe:(current&&current.creeLe)||new Date() }); });
