...
async function renderRemise(remise){
  ...
  const fit=()=>{const scale=Math.min(1,viewport.clientWidth/(exercise.canvas.width||650));canvas.style.transform=`scale(${scale})`;viewport.style.height=`${(exercise.canvas.height||400)*scale}px`;};fit();new ResizeObserver(fit).observe(viewport);
}
