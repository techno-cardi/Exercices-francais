...
function layoutCanvas(root){
  const nodes=[...root.querySelectorAll('.canvas-element')],fields=nodes.filter(node=>node.classList.contains('canvas-input')||node.classList.contains('canvas-word_inputs'));
  ...
  for(const field of fields){const anchored=field.dataset.anchorAligned==='1',left=anchored?Number(field.dataset.dynamicLeft)||0:Math.max(Number(field.dataset.baseLeft)||0,Number(field.dataset.dynamicLeft)||0),top=anchored?Number(field.dataset.dynamicTop)||0:Math.max(Number(field.dataset.baseTop)||0,Number(field.dataset.dynamicTop)||0),width=Math.max(Number(field.dataset.baseWidth)||40,Number(field.dataset.dynamicWidth)||0);field.style.left=`${left}px`;field.style.top=`${top}px`;field.style.width=`${width}px`;}
  for(const field of fields){const left=field.dataset.anchorAligned==='1'?Number(field.dataset.dynamicLeft)||0:Math.max(Number(field.dataset.baseLeft)||0,Number(field.dataset.dynamicLeft)||0),baseWidth=Number(field.dataset.baseWidth)||40,right=left+(Number(field.dataset.dynamicWidth)||baseWidth),top=field.dataset.anchorAligned==='1'?Number(field.dataset.dynamicTop)||0:Math.max(Number(field.dataset.baseTop)||0,Number(field.dataset.dynamicTop)||0),height=Number(field.dataset.baseHeight)||32;for(const other of nodes){if(other===field||!other.classList.contains('canvas-label'))continue;const otherLeft=Number(other.dataset.baseLeft)||0,otherTop=Number(other.dataset.baseTop)||0,otherHeight=Number(other.dataset.baseHeight)||32,sameRow=Math.min(top+height,otherTop+otherHeight)-Math.max(top,otherTop)>Math.min(height,otherHeight)*.35;if(sameRow&&otherLeft>=left+baseWidth-4)other.style.left=`${Math.max(otherLeft,right+8)}px`;}}}
function splitPlaceholderLabel(label,raw,matches,fields){const labelLeft=Number(label.dataset.baseLeft)||0,labelTop=Number(label.dataset.baseTop)||0,labelHeight=Number(label.dataset.baseHeight)||32;label.dataset.placeholderSplit='1';label.dataset.placeholderFields=fields.map(field=>field.dataset.widgetId||'').join('|');label.innerHTML='';label.style.width='max-content';label.style.whiteSpace='nowrap';const parts=raw.split(/[\s\u00a0]{2,}/gu);for(const part of parts){const span=document.createElement('span');span.className='canvas-label-part';span.textContent=part.trim();span.style.position='absolute';span.style.top='0';label.append(span);}reflowPlaceholderLabel(label,[...label.parentElement.querySelectorAll('.canvas-element')]);fields.forEach(field=>{field.dataset.dynamicTop=String(labelTop+(labelHeight-(Number(field.dataset.baseHeight)||32))/2);});}
