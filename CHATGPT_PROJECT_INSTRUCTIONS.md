...
2. The extension offers a simple action such as `Corriger avec ChatGPT` / `Copier les réponses pour ChatGPT`.
3. The copied content already contains the question, point value, responses, and invisible/automatic instructions needed for ChatGPT to return importable results. Kevin should be able to talk normally afterward.
4. Kevin may freely calibrate the correction in chat: give a rubric, examples, change one grade, say the grading is too generous, ask to regrade everybody, request comments or no comments, etc.
5. When he is satisfied, he must be able to send the grades from the ChatGPT response back into the exact Formative question with minimal effort.
6. The same Chrome extension may add an `Envoyer les résultats dans Formative` button on ChatGPT pages. This is preferable to adding another backend connector.
7. Before writing, show a local preview with student names and old -> new grades. Nothing is written until Kevin confirms.
8. Notes and comments are independent. Comments are optional.
9. Robust matching must not silently guess. Name matching can be used for the user-facing table, but ambiguous or unmatched students must be blocked or resolved locally before publishing.
10. At the end of the whole evaluation, import only the **global Formative result** into Gestion des notes, then send that result to Mozaïk.
...
The experimental `v0.9.0` and `v0.9.1-beta` builds were prototypes for ChatGPT correction. They are not the production baseline and should not be promoted without controlled testing.
